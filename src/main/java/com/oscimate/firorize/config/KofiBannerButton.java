package com.oscimate.firorize.config;

import net.minecraft.client.MinecraftClient;
import net.minecraft.client.gui.DrawContext;
import net.minecraft.client.gui.screen.ConfirmLinkScreen;
import net.minecraft.client.gui.screen.Screen;
import net.minecraft.client.gui.widget.ButtonWidget;
import net.minecraft.client.texture.Sprite;
import net.minecraft.client.texture.SpriteAtlasTexture;
import net.minecraft.client.util.SpriteIdentifier;
import net.minecraft.util.Identifier;

/**
 * Slim, persistent "Support Firorize on Ko-fi" banner on the entry {@link ConfigScreen}. Designed to
 * be noticeable but unobtrusive: a dark pill with the Ko-fi-red accent border and cup icon that fills
 * with Ko-fi red on hover to invite a click. Pressing it opens the Ko-fi page via the vanilla
 * confirm-link dialog.
 */
public class KofiBannerButton extends ButtonWidget {
    private static final int KOFI_RED = 0xFFFF5E5B;
    private static final int IDLE_BG = 0xFF2A1416;

    protected KofiBannerButton(Screen parent, int x, int y, int width, int height) {
        super(x, y, width, height, net.minecraft.text.Text.translatable("firorize.donate.banner"),
                openKofi(parent), DEFAULT_NARRATION_SUPPLIER);
    }

    // Concretely-typed PressAction so the super(...) call doesn't trip lambda overload inference.
    private static PressAction openKofi(Screen parent) {
        return button -> ConfirmLinkScreen.open(parent, DonationTracker.KOFI_URL);
    }

    @Override
    @SuppressWarnings("deprecation") // BLOCK_ATLAS_TEXTURE is deprecated but still the supported atlas id in 1.21
    protected void renderWidget(DrawContext context, int mouseX, int mouseY, float delta) {
        int x1 = getX(), y1 = getY(), x2 = getX() + getWidth();
        boolean hovered = isHovered();

        context.fill(x1, y1, x2, getY() + getHeight(), hovered ? KOFI_RED : IDLE_BG);
        context.drawBorder(x1, y1, getWidth(), getHeight(), KOFI_RED);

        int iconSize = 12;
        int iconX = x1 + 8;
        int iconY = y1 + (getHeight() - iconSize) / 2;
        Sprite kofi = new SpriteIdentifier(SpriteAtlasTexture.BLOCK_ATLAS_TEXTURE, Identifier.of("firorize:block/kofi")).getSprite();
        context.drawSprite(iconX, iconY, 10, iconSize, iconSize, kofi);

        int centerX = (iconX + iconSize + x2) / 2;
        context.drawCenteredTextWithShadow(MinecraftClient.getInstance().textRenderer, getMessage(),
                centerX, y1 + (getHeight() - 8) / 2, hovered ? 0xFFFFFFFF : 0xFFFFC8C6);
    }
}
