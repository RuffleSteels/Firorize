...
class PresetListWidget
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        if (reorderMode && button == 0) {
            PresetEntry e = getEntryAtPosition(mouseX, mouseY);
            if (e != null) {
    ...
    public boolean mouseDragged(double mouseX, double mouseY, int button, double deltaX, double deltaY) {
        if (reorderMode && dragFromIndex >= 0) {
            PresetEntry over = getEntryAtPosition(mouseX, mouseY);
            int target = over != null ? children().indexOf(over)
                    : (mouseY < getY() ? 0 : children().size() - 1);
    ...
    public class PresetEntry
        ...
        public void render(DrawContext context, int index, int y, int x, int entryWidth, int entryHeight, int mouseX, int mouseY, boolean hovered, float tickDelta) {
            ...
            if (reorderMode) {
                ...
                // Active checkbox: green when applied, red when not.
                int cbSize = 10;
                int cbX = x + 4;
                int cbY = y + (entryHeight - cbSize) / 2;
                ...
                context.drawBorder(cbX, cbY, cbSize, cbSize, cbHover ? 0xFFFFFFFF : (active ? 0xFF59C24E : 0xFFC25555));
            ...
            // Marker for imported profiles (drawn after the checkbox; the name is left-aligned). Built-in
            ...
            int deleteReserve = entryHeight + 8;
            ...
            context.drawBorder(badgeX, badgeY, badgeW, 11, new Color(0x5A / 255f, 0x5A / 255f, 0x5A / 255f, dim).getRGB());
