...
public class OnlinePresetListWidget extends AbstractWidget {
    ...
    private void renderRow(GuiGraphicsExtractor context, Row row, int x, int y, int h, int mouseX, int mouseY) {
        ...
        int badgeW = 0;
        if (type >= 0) {
            Component badge = Component.translatable(TYPE_BADGE_KEYS[type]);
            ...
            context.outline(badgeX, badgeY, bw, 11, 0xFF5A5A5A);
            int[] tc = TYPE_BADGE_COLORS[type];
            context.text(font, badge, badgeX + 3, badgeY + 2, 0xFF000000 | (tc[0] << 16) | (tc[1] << 8) | tc[2], false);
            badgeW = bw + 6;
        }
    ...
    private Component primaryLabel(Row row) {
        return switch (row.kind) {
            case BROWSE_IMPORT, INBOX_IMPORT, BUILTIN_IMPORT -> Component.translatable("firorize.config.button.importPreset");
    ...
    private void importPreset(OnlinePreset preset, String author, int dismissSendId, boolean builtin) {
        ...
        int type = com.oscimate.firorize.config.ConfigManager.deriveType(profile);
    ...
    private final class Row {
        ...
        int type() {
            if (cachedType == -2) {
                ...
                cachedType = profile == null ? -1 : com.oscimate.firorize.config.ConfigManager.deriveType(profile);
            }
            return cachedType;
        }
